...
<div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
    <div class="flex">
        ...
        <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800">Aucun restaurant associé</h3>
            <div class="mt-2 text-sm text-red-700">
                <p>Votre compte n'est associé à aucun restaurant. Veuillez contacter l'administrateur.</p>
            </div>
        </div>
    </div>
</div>
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Organisez vos produits par catégories</p>
        </div>
        <div class="mt-4 sm:mt-0">
            <a href="{{ route('restaurant.categories.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['average_price'] ?? 0, 0, ',', ' ') }} FCFA</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $category->products_count }} produits</div>
                            <div class="text-sm text-gray-500">{{ $category->active_products_count }} actifs</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">
                                @if($category->products_count > 0)
                                    {{ currency($category->products()->avg('price') ?? 0) }}
                                @else
                                    Aucun produit
                                @endif
                            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('restaurant.categories.edit', $category->id) }}" class="text-orange-600 hover:text-orange-900" title="Modifier">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <button onclick="toggleCategoryStatus({{ $category->id }})" class="text-blue-600 hover:text-blue-900" title="Changer le statut">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="text-gray-500">
                                ...
                                <div class="mt-6">
                                    <a href="{{ route('restaurant.categories.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500">
...
function toggleCategoryStatus(categoryId) {
    console.log('Changement de statut de la catégorie:', categoryId);
